...
| `sentence_cache` | Pre-generated sentences for today's cards | PK: `(chat_id, topic, word_id, generated_for)`; `sentence` |
...
**Sentence cache:** At startup and every midnight, `_prefill_sentence_cache` pre-generates `DAILY_SLOTS * 2` sentences per user and stores them in `sentence_cache`. `send_card` checks the cache first and only calls Gemini on a miss. Cache rows older than 7 days are purged at prefill time. The `generated_for` column is today's date (YYYY-MM-DD) so sentences regenerate each day.

**Card delivery (`send_card`):** Looks up cached sentence; if missing, calls `generate_sentence()` (gemini-2.5-flash) with a 15-second `asyncio.wait_for` timeout. If the call times out or raises, the card is sent without a sentence — the word is never suppressed. `format_sentence_words` wraps the main word in `<u><b>…</b></u>` and any weaved-in retired words in `<u>…</u>` (HTML parse mode). The `hint` field (if set) is shown as an `<i>italic</i>` line between the word and the sentence. Retired words are only woven in when `len(retired) >= MIN_RETIRED_FOR_WEAVING` (3).
