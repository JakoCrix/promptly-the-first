"""
Generate and store example sentences for the Korean TOPIK corpus.

For each word in korean_* topics:
  - Generates N sentences via Gemini (one API call per word)
  - Writes immediately to the word_sentences DB table
  - Appends to CSV as a backup (data/word_sentences/korean.csv)

Resumable: words already having >= count sentences in the DB are skipped.

Usage:
    python scripts/sentences/korean.py
    python scripts/sentences/korean.py --topic korean_topik_1
    python scripts/sentences/korean.py --count 10 --delay 0.5
"""
...
OUTPUT_CSV = os.path.join(PROJECT_ROOT, "data", "word_sentences", "korean.csv")
...
def _fetch_words(topic_filter: str | None) -> list[dict]:
    ...
    try:
        if topic_filter:
            ...
            rows = conn.execute(
                "SELECT topic, word_id, word, definition FROM corpus "
                "WHERE topic LIKE 'korean_%' ORDER BY topic, frequency_rank",
            ).fetchall()
...
def _generate_sentences(word: str, definition: str | None, count: int) -> list[str]:
    definition_line = f'Meaning: "{definition}"\n' if definition else ""
    prompt = (
        f'Generate exactly {count} natural example sentences in Korean using the word or phrase "{word}".\n'
        f"{definition_line}"
        f"Each sentence should be distinct and useful for a Korean language learner.\n"
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Generate sentences for the Korean TOPIK corpus")
