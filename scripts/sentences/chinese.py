"""
Generate and store example sentences for the Chinese HSK corpus.

For each word in chinese_hsk_* topics:
  - Generates N sentences via Gemini (one API call per word)
  - Writes immediately to the word_sentences DB table
  - Appends to CSV as a backup (data/word_sentences/chinese.csv)

Resumable: words already having >= count sentences in the DB are skipped.

Usage:
    python scripts/sentences/chinese.py
    python scripts/sentences/chinese.py --topic chinese_hsk_1
    python scripts/sentences/chinese.py --count 10 --delay 0.5
"""
...
def _load_done(min_count: int) -> set[tuple[str, str]]:
    """Return (topic, word_id) pairs that already have >= min_count sentences in the DB."""
    conn = _get_conn()
    try:
        rows = conn.execute(
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Generate sentences for the Chinese HSK corpus")
    parser.add_argument("--topic", default=None, help="Limit to one topic (e.g. chinese_hsk_1)")
