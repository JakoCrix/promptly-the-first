"""
Generate and store example sentences for the Spanish PCIC corpus.

For each word in spanish_* topics:
  - Generates N sentences via Gemini (one API call per word)
  - Writes immediately to the word_sentences DB table
  - Appends to CSV as a backup (data/word_sentences/spanish.csv)

Resumable: words already having >= count sentences in the DB are skipped.

Usage:
    python scripts/sentences/spanish.py
    python scripts/sentences/spanish.py --topic spanish_pcic_a1
    python scripts/sentences/spanish.py --count 10 --delay 0.5
"""
...
OUTPUT_CSV = os.path.join(PROJECT_ROOT, "data", "word_sentences", "spanish.csv")
...
def _fetch_words(topic_filter: str | None) -> list[dict]:
    ...
    try:
        if topic_filter:
            ...
            rows = conn.execute(
                "SELECT topic, word_id, word, hint FROM corpus "
                "WHERE topic LIKE 'spanish_%' ORDER BY topic, frequency_rank",
            ).fetchall()
...
def _generate_sentences(word: str, hint: str | None, count: int) -> list[str]:
    prompt = (
        f'Generate exactly {count} natural example sentences in Spanish using the word or phrase "{word}".\n'
        f"Each sentence should be distinct and useful for a Spanish language learner.\n"
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Generate sentences for the Spanish PCIC corpus")
    parser.add_argument("--topic", default=None, help="Limit to one topic (e.g. spanish_pcic_a1)")
